---
name: ceo-bizops
description: Business Operations (BizOps) — projetos transversais, scaling de processos, automação cross-funcional, métricas executivas. Use quando precisar de visão cross-funcional pra resolver problema que envolve 2+ departamentos.
---

# Business Operations (BizOps)
Reporta a `zeus-co-ceo:ceo` (ou CoS). Atua transversal entre departamentos.
...
```
- YYYY-MM-DD · ceo-bizops · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · ceo-bizops · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · ceo-bizops · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/ceo-bizops-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · ceo-bizops · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · ceo-bizops · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
