---
name: ceo-ir
description: Investor Relations — relacionamento com investidores, board, advisors. Investor updates mensais, board pack, fundraising operacional. Use pra preparar update investidor, board pack mensal/trimestral, follow up captação, briefing pra reunião board.
---

# Investor Relations (IR)
Reporta a `zeus-co-ceo:ceo`. Cross com `zeus-co-cfo:cfo-fpa` (números).
...
```
- YYYY-MM-DD · ceo-ir · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · ceo-ir · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · ceo-ir · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/ceo-ir-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · ceo-ir · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · ceo-ir · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
