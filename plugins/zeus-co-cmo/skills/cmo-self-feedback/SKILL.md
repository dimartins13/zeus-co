---
name: cmo-self-feedback
description: Camada 2 do Sistema Vivo — Audit semanal das skills do CMO (Marketing). Lê todos os `_Pulse/skill-feedback/cmo*-YYYY-MM-DD.md` da semana, identifica padrões de gap/sucesso, e gera relatório em `_Improvements/cmo-RADAR-YYYY-WW.md`. Use SEMPRE pra "auditar CMO", "feedback Marketing", "evolução CMO", "como está o Marketing", "melhorar CMO", "saúde do Marketing", "skill health CMO". Invocação típica: semanal (cron sexta) OU manual sob demanda.
---

# CMO Self-Feedback — Camada 2 do Sistema Vivo

## Identidade
Audit reflexivo das skills do CMO (Marketing). Não opero — observo e relato.
...
1. **Skill feedback files** (Camada 1):
   ```
   ~/Documents/Claude/Projects/_Pulse/skill-feedback/cmo-*.md
   ~/Documents/Claude/Projects/_Pulse/skill-feedback/cmo-*-*.md   # specialists
   ```
...
### 1. Agregação
- Lista todas as invocações da família `cmo*` na semana
...
### 4. Output: `_Improvements/cmo-RADAR-YYYY-WW.md`
Estrutura:
```markdown
# RADAR CMO — Semana YYYY-WW
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/cmo-self-feedback-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · cmo-self-feedback · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · cmo-self-feedback · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
...
### ⬇️ Downstream
  ...
  - `_Improvements/cmo-RADAR-YYYY-WW.md` (output)
...
Esta skill é **capability reutilizável** pra avaliar a família CMO em qualquer escopo. Como opera sobre logs agregados, lê arquivos de TODAS as empresas (não de uma só).

## Fim de sessão (obrigatório — 3 outputs hard + 1 opcional)

### 1. LEARNINGS.md (portfolio global, em `~/Documents/Claude/Projects/LEARNINGS.md`)
- YYYY-MM-DD · cmo-self-feedback · [lição] · [por que importa]

### 2. BACKLOG.md (portfolio global)
- [P0|P1|P2] · [ação derivada do RADAR] · Owner

### 3. _LEDGER.md (portfolio global)
- YYYY-MM-DD HH:MM · cmo-self-feedback · audit-weekly · ~N turnos · `_Improvements/cmo-RADAR-YYYY-WW.md`

### 4. _Inbox.md (opcional)
Findings 🔴 Type 1 sempre viram entry em _Inbox global pra Diego ver.

**Fallback:** `_SessionRecaps/YYYY-MM-DD-cmo-feedback.md`.
