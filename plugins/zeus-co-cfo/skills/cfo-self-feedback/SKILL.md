---
name: cfo-self-feedback
description: Camada 2 do Sistema Vivo — Audit semanal das skills do CFO (Finance). Lê todos os `_Pulse/skill-feedback/cfo*-YYYY-MM-DD.md` da semana, identifica padrões de gap/sucesso, e gera relatório em `_Improvements/cfo-RADAR-YYYY-WW.md`. Use SEMPRE pra "auditar CFO", "feedback Finance", "evolução CFO", "como está o Finance", "melhorar CFO", "saúde do Finance", "skill health CFO". Invocação típica: semanal (cron sexta) OU manual sob demanda.
---

# CFO Self-Feedback — Camada 2 do Sistema Vivo

## Identidade
Audit reflexivo das skills do CFO (Finance). Não opero — observo e relato.
...
1. **Skill feedback files** (Camada 1):
   ```
   ~/Documents/Claude/Projects/_Pulse/skill-feedback/cfo-*.md
   ~/Documents/Claude/Projects/_Pulse/skill-feedback/cfo-*-*.md   # specialists
   ```
...
### 1. Agregação
- Lista todas as invocações da família `cfo*` na semana
...
### 4. Output: `_Improvements/cfo-RADAR-YYYY-WW.md`
Estrutura:
```markdown
# RADAR CFO — Semana YYYY-WW
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/cfo-self-feedback-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · cfo-self-feedback · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · cfo-self-feedback · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
...
### ⬇️ Downstream
  ...
  - `_Improvements/cfo-RADAR-YYYY-WW.md` (output)
...
Esta skill é **capability reutilizável** pra avaliar a família CFO em qualquer escopo. Como opera sobre logs agregados, lê arquivos de TODAS as empresas (não de uma só).

## Fim de sessão (obrigatório — 3 outputs hard + 1 opcional)

### 1. LEARNINGS.md (portfolio global, em `~/Documents/Claude/Projects/LEARNINGS.md`)
- YYYY-MM-DD · cfo-self-feedback · [lição] · [por que importa]

### 2. BACKLOG.md (portfolio global)
- [P0|P1|P2] · [ação derivada do RADAR] · Owner

### 3. _LEDGER.md (portfolio global)
- YYYY-MM-DD HH:MM · cfo-self-feedback · audit-weekly · ~N turnos · `_Improvements/cfo-RADAR-YYYY-WW.md`

### 4. _Inbox.md (opcional)
Findings 🔴 Type 1 sempre viram entry em _Inbox global pra Diego ver.

**Fallback:** `_SessionRecaps/YYYY-MM-DD-cfo-feedback.md`.
