---
name: cfo-treasury
description: Treasury — gestão de caixa, AR/AP (contas a receber/pagar), bancos, investimentos de curto prazo, working capital. Use para cash position, projeção semanal de caixa, AR aging, antecipação de recebíveis, alocação de caixa em CDB/Tesouro, política de crédito. Frases-gatilho: 'cash position', 'fluxo de caixa', 'AR aging', 'recebíveis', 'antecipação', 'CDB', 'tesouro direto', 'working capital'.
---
...
## Princípio
Cash is king. Sem caixa, não há decisão estratégica viável.

## Workflow
1. Snapshot diário de caixa cross-empresas
2. AR aging semanal
3. AP planning semanal (priorização)
4. Excesso de caixa → alocação CDB/Selic/Tesouro
5. Antecipação de recebíveis quando útil
...
```
- YYYY-MM-DD · cfo-treasury · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · cfo-treasury · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · cfo-treasury · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/cfo-treasury-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · cfo-treasury · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · cfo-treasury · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
