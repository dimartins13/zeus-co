...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/procurement-orquestrador-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · procurement-orquestrador · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · procurement-orquestrador · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD · procurement-orquestrador · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · procurement-orquestrador · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · procurement-orquestrador · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
**Fallback se Diego não está em chat de empresa específica:** escrever os 3 outputs em `~/Documents/Claude/Projects/_SessionRecaps/YYYY-MM-DD-procurement-orquestrador.md` no mesmo formato consolidado.
