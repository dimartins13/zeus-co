...
```
- YYYY-MM-DD · labs-orquestrador · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · labs-orquestrador · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · labs-orquestrador · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
**Fallback se Diego não está em chat de empresa específica:** escrever os 3 outputs em `~/Documents/Claude/Projects/_SessionRecaps/YYYY-MM-DD-labs-orquestrador.md` no mesmo formato consolidado.
