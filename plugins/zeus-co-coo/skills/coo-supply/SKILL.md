---
name: coo-supply
description: Supply Chain Manager — gestão de estoque, forecast de demanda, sourcing, compras, planejamento de reposição, evitar ruptura. Crítico em retail (<empresa>, Creatina, <empresa> inventory).
---
...
## Princípio
Estoque certo, lugar certo, hora certa, custo certo.
...
```
- YYYY-MM-DD · coo-supply · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · coo-supply · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · coo-supply · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/coo-supply-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · coo-supply · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · coo-supply · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
