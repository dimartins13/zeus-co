---
name: cto-qa
description: Quality Engineering — testes, validação, prevenção de regressão, automação de QA. Use pra desenhar test plan, escrever caso teste, automatizar smoke test, validar release antes de deploy.
---

# Quality Engineering
Reporta a `zeus-co-cto:cto-vp-eng`.
...
```
- YYYY-MM-DD · cto-qa · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · cto-qa · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · cto-qa · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/cto-qa-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · cto-qa · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · cto-qa · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
