---
name: cco-self-feedback
description: Camada 2 do Sistema Vivo — Audit semanal das skills do CCO (Creative). Lê todos os `_Pulse/skill-feedback/cco*-YYYY-MM-DD.md` da semana, identifica padrões de gap/sucesso, e gera relatório em `_Improvements/cco-RADAR-YYYY-WW.md`. Use SEMPRE pra "auditar CCO", "feedback Creative", "evolução CCO", "como está o Creative", "melhorar CCO", "saúde do Creative", "skill health CCO". Invocação típica: semanal (cron sexta) OU manual sob demanda.
---

# CCO Self-Feedback — Camada 2 do Sistema Vivo

## Identidade
Audit reflexivo das skills do CCO (Creative). Não opero — observo e relato.
...
1. **Skill feedback files** (Camada 1):
   ```
   ~/Documents/Claude/Projects/_Pulse/skill-feedback/cco-*.md
   ~/Documents/Claude/Projects/_Pulse/skill-feedback/cco-*-*.md   # specialists
   ```
...
### 1. Agregação
- Lista todas as invocações da família `cco*` na semana
...
### 4. Output: `_Improvements/cco-RADAR-YYYY-WW.md`
Estrutura:
```markdown
# RADAR CCO — Semana YYYY-WW
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/cco-self-feedback-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · cco-self-feedback · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · cco-self-feedback · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
...
### ⬇️ Downstream
  ...
  - `_Improvements/cco-RADAR-YYYY-WW.md` (output)
...
Esta skill é **capability reutilizável** pra avaliar a família CCO em qualquer escopo. Como opera sobre logs agregados, lê arquivos de TODAS as empresas (não de uma só).

## Fim de sessão (obrigatório — 3 outputs hard + 1 opcional)

### 1. LEARNINGS.md (portfolio global, em `~/Documents/Claude/Projects/LEARNINGS.md`)
- YYYY-MM-DD · cco-self-feedback · [lição] · [por que importa]

### 2. BACKLOG.md (portfolio global)
- [P0|P1|P2] · [ação derivada do RADAR] · Owner

### 3. _LEDGER.md (portfolio global)
- YYYY-MM-DD HH:MM · cco-self-feedback · audit-weekly · ~N turnos · `_Improvements/cco-RADAR-YYYY-WW.md`

### 4. _Inbox.md (opcional)
Findings 🔴 Type 1 sempre viram entry em _Inbox global pra Diego ver.

**Fallback:** `_SessionRecaps/YYYY-MM-DD-cco-feedback.md`.
