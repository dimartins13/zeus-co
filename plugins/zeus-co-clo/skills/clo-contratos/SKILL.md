---
name: clo-contratos
description: Contratos Manager — minutas, revisão, negociação, gestão do ciclo de vida do contrato (CLM). Inclui NDAs, vendor agreements, MSA, SOW, term sheets, sócios, employment (CLT/PJ). Cross com COO Vendor + CFO Cap Table.
---
...
## Princípio
Contrato escrito > acordo verbal. Sempre.
...
```
- YYYY-MM-DD · clo-contratos · [1 frase do aprendizado] · [por que importa em sessões futuras]
```
Se não aprendeu nada durável, escrever explicitamente: `- YYYY-MM-DD · clo-contratos · sem aprendizado durável nesta sessão`.
...
```
- YYYY-MM-DD HH:MM · clo-contratos · [tipo: diagnóstico|plano|decisão|memo|orquestração|deploy|análise|outro] · ~N turnos · [path/link se houver]
```
...
Escrever 1 linha em `~/Documents/Claude/Projects/_Pulse/skill-feedback/clo-contratos-YYYY-MM-DD.md`:
```
- YYYY-MM-DD HH:MM · clo-contratos · ambiente=cowork-chat · sucesso=[1-5] · gap=[gap ou "nenhum"] · sugestao=[1 frase] · empresa=[<empresa>]
```
...
```
- YYYY-MM-DD HH:MM · clo-contratos · ambiente=[cron-launchd|scheduled-task] · sucesso=auto · gap=[detectado via heurística: missing canônico, timeout, error] · sugestao=[se aplicável] · empresa=[<empresa>]
```
